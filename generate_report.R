...
#                      (Email step is optional via SEND_EMAIL env var)
# ---------------------------------------------------------------------------

# 0 ── PACKAGES ──────────────────────────────────────────────────────────────
required <- c(
  "tidyverse", "lubridate", "httr2", "httr", "jsonlite", "glue",
  "pagedown", "rmarkdown", "RPostgres", "DBI", "base64enc", "tidytext", "magrittr"
)
...
# Require DB / Supabase / OpenAI always
stopifnot(SB_HOST != "", SB_URL != "", SB_STORAGE_KEY != "", OPENAI_KEY != "")
# Only require Mailjet creds if we’re emailing
...
# 4 ── PRE-PROCESS TWEETS (last 7 days) -------------------------------------
tweets <- twitter_raw |>
...
clean_gpt_output <- function(txt) {
  txt <- gsub("\\((https?://[^)\\s]+)\\s*\\(\\1\\)\\)", "(\\1)", txt, perl = TRUE)   # squash "(url (url))"
  keep <- function(l) { l <- trimws(l); !(l == "" || grepl("^https?://", l) || grepl("^\\(", l)) }
  lines <- strsplit(txt, "\n", fixed = TRUE)[[1]]
...
launches_summary <- raw %>%
  clean_gpt_output() %>%
  gsub("<(https?://[^>\\s]+)>", "\\1", ., perl = TRUE) %>%                         # drop any angle-wrapped links
  gsub("(?<!\\))\\s(https?://[^\\s)]+)\\s*$", " (\\1)", ., perl = TRUE)            # wrap bare URL at end with (...)

overall_summary <- launches_summary

# 6 ── SECTION 2 – NUMERIC INSIGHTS, CONTENT TYPE, HASHTAGS ------------------
content_tbl <- df2 |>
  mutate(post_type = stringr::str_to_title(tweet_type)) |>
  group_by(post_type) |>
  summarise(
...
hashtag_block <- df2 |>
  ...
  unnest(hashtag) |>
  group_by(hashtag) |>
  summarise(n_tweets = n(), avg_ER = mean(engagement_rate, na.rm = TRUE), .groups = "drop") |>
  filter(n_tweets >= 3) |>
  arrange(desc(avg_ER)) |>
  slice_head(n = 5) |>
...
prompt2 <- glue(
  "You are an experienced social-media analyst. Each line in **Data A** has YYYY-MM-DD HH:MM | ER=% | tweet_text. **Data B** five-number summaries; **Data C** content types; **Data D** hashtags; **Data E** best-time.",
  "\n### Tasks\n",
...
tidy_tokens <- bind_rows(
  ...
  df_tier |> unnest_tokens(word, text, token = "ngrams", n = 2) |> separate_rows(word, sep = " ")
) |>
...
week_lines <- df |>
  ...
  mutate(line = glue("{format(publish_dt, '%Y-%m-%d %H:%M')} | ER={round(engagement_rate,4)}% | {str_trunc(text, 200)} | {tweet_url}")) |>
  pull(line)
...
if (!nzchar(chrome)) chrome <- pagedown::find_chrome()
...
make_md_links <- function(txt) {
  pattern <- "(?<!\\]\\()https?://\\S+"
  stringr::str_replace_all(txt, pattern, function(m) sprintf("<a href=\"%s\">%s</a>", m, m))
}

final_report <- glue("{overall_summary}\n\n{overall_summary2}\n\n{overall_summary3}\n\n## Weekly Round-up\n\n{overall_summary4}") |>
  stringr::str_replace_all("\\$", "\\\\$") |>
  make_md_links()

writeLines(c(
  "<style> a, a:visited { color:#1a0dab !important; text-decoration:underline; } </style>",
  "",
  "# Weekly Summary",
  "",
  final_report
